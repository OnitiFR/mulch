...
func ListReplicationController(req *server.Request) {
	...
	for _, s := range states {
		...
		entry := common.APIReplicationEntry{
			...
			NextSyncTime:      req.App.ReplicationMgr.GetNextSyncTime(vmName.ID()),
...
func SyncReplicationController(req *server.Request) {
	...
	err := req.App.ReplicationReceiver.ApplyBlocks(vmName, replicaOrigin(req), fullCopy, req.HTTP.Body)
	if err != nil {
		...
		if errors.Is(err, server.ErrReplicaPromoted) {
