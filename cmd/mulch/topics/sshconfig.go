...
# Note: ForwardAgent is set for all hosts but actual forwarding is done
# by the SSH proxy and is disabled by default (see 'trust' command).
...
`)
	...
	for _, vm := range *conf.vmList {
		...
		file.WriteString("    ForwardAgent yes\n")
		file.WriteString("\n")
